...
request due to privacy concerns, but anonymized bam files and processed DNA methylation calls are available through GEO (GSE245745). 
All results from the paper can be produced using only DNA methylation counts data.
...
Notice that MacOS systems might require the additional installation of `Xcode`, `gfortran`, `StanHeaders`, and R dev binaries using `macrtools`.
...
A singularity image containing `NEMOcode` and the required packages can be build using the following recipe:

```{singularity, eval = FALSE}
Bootstrap: docker
From: ubuntu:22.04
IncludeCmd: yes
